...
def test_filter_perf_bench_100_comps_under_50ms() -> None:
    ...
    comps = [
        _comp(item_id=str(i), title=f"MDL-A03 listing {i}") for i in range(100)
    ]
    # Warm the cache.
...
def test_caddy_detection_features_specifics() -> None:
    ...
    own = _own(specifics={"MPN": ["MDL-A03"], "Form Factor": ['2.5"'], "Features": ["Caddy", "Hot Swap"]})
    comps = [_comp(title="MDL-A03 bare drive no caddy")]
    survivors, audit = filter_low_quality_competitors(comps, own_listing=own)
    assert audit["dropped_reasons"]["caddy_mismatch"] == 1
...
def test_pipeline_audit_flat_six_keys() -> None:
    ...
    comps = [
        _comp(item_id=str(i), title=f"MDL-A03 listing {i}") for i in range(8)
    ]
