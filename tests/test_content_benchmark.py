...
def test_score_bundle_keyword_no_score_impact() -> None:
    ...
    text in the title.
    """
    score = score_apple_to_apple(_own(), _comp(title='MDL-A03 with caddy 2.5"'))
    assert score == 1.0


def test_score_form_factor_mismatch() -> None:
    """Comp is 3.5" not 2.5" → FF dim fails → 0.75."""
    score = score_apple_to_apple(_own(), _comp(title='MDL-A03 3.5 SAS'))
    assert score == 0.75
...
def test_filter_clean_keeps_only_above_threshold() -> None:
    ...
    comps = [
        _comp(),  # 1.0
        _comp(title="MDL-A03 with caddy 2.5"),  # 1.0 (bundle dim removed Phase 1.6)
        _comp(title='MDL-A03 3.5 SAS'),  # MPN+Cond+age, no FF → 0.75
