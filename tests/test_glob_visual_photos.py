"""Tests for ebay.photos.glob_visual_photos + create_listing triple-glob (#25 stub-body correction).
...
from ebay.photos import glob_visual_photos
from server import _glob_label_photos
...
def test_visual_glob_finds_three_patterns(tmp_path: Path) -> None:
    ...
    out = glob_visual_photos(tmp_path)
...
def test_label_then_visual_glob_combined_order(tmp_path: Path) -> None:
    ...
    visual = glob_visual_photos(tmp_path)
