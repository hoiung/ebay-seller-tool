...
- **Default-shipping policy POISONED (durable rule)**: `~/.claude/projects/-home-hoiung-DevProjects/memory/feedback_ebay_default_shipping_poisoned.md` — after the 2026-05-26 permanent fix, `build_add_payload` + `build_revise_payload` attach NO `SellerProfiles` block at all (3-incident shipping-poison history; eBay auto-fills missing policy slots from account defaults, destroying inline shipping). `_build_seller_profiles_block` raises `NotImplementedError`. `_REQUIRED_SELLER_PROFILE_ENV_VARS = ()`. `scripts/apply_returns_policy.py` is hard-refused without `--i-acknowledge-shipping-corruption`. Source of truth: account-level eBay Simple Delivery + manually-set free shipping.

### Comp-filter quality config (Issue #14 + #444 Part B)
- `config/pricing_and_content.yaml` `comp_filter:` block (PUBLIC — generic only) — Layer-1 binary thresholds + Layer-2 deductions + generic hard-reject categories (broken_or_parts/bundle) + condition_equivalence (numeric Phase 2.3 classes). The product-specific taxonomy — series_names (+ the series-distinctness HARD CONTRACT), caddy_mismatch_patterns, sibling_allowlist, preserved_phrases, mandatory_by_drive_class, and the storage-specific hard-rejects (external/wrong_category) — relocated to the PRIVATE ebay-ops overlay, deep-merged at runtime via `EBAY_LISTING_DATA_DIR` (#45 category-agnostic refactor)
