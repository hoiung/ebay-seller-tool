...
- `config/pricing_and_content.yaml` `comp_filter:` block — Layer-1 binary thresholds + Layer-2 deductions + 4 hard-reject regex categories (broken/external/wrong_category/bundle) + caddy_mismatch_patterns + condition_equivalence (numeric Phase 2.3 classes) + series_names (Fabrikam HARD CONTRACT et al)
